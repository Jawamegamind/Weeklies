    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        for stmt in SCHEMA.strip().split(";"):
            if stmt.strip():
                cur.execute(stmt)
        cur.execute(
            'INSERT INTO "User"(usr_id, first_name, last_name, email, phone) VALUES (100,"Linus","Torvalds","linus@example.com","555-2222")'
        )
        cur.execute(
            'INSERT INTO "Restaurant"(rtr_id, name, address, city, state, zip, phone) VALUES (10,"Code Cafe","1 Kernel Way","Raleigh","NC","27606","555-3333")'
        )

        items = [
            {"qty": 1, "name": f"Item {i}", "unit_price": 1.0, "line_total": 1.0} for i in range(60)
        ]
        details = {
            "placed_at": "2025-11-05T12:00:00",
            "items": items,
            "charges": {"subtotal": 60, "tax": 4.8, "total": 64.8},
        }
        cur.execute(
            'INSERT INTO "Order"(ord_id, rtr_id, usr_id, details, status) VALUES (1,10,100,?,?)',
            (json.dumps(details), "paid"),
        )
        con.commit()
        con.close()
